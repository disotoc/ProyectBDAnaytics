...
dropship['Día de Order Date'] = dropship['Día de Order Date'].apply(
    lambda self: datetime.strptime(self, "%d de %B de %Y")).astype('datetime64[ns]')
dropship['Día de Po Date'] = dropship['Día de Po Date'].apply(
    lambda self: datetime.strptime(self, "%d de %B de %Y")).astype('datetime64[ns]')
...
dropship = dropship.astype({'Order Number': 'string',
                            'Po Number': 'string'})
dropship = dropship[(dropship.Client == 'Latam') | (dropship.Client == 'Latam CL') | (dropship.Client == 'Latam PE') | (
            dropship.Client == 'Latam CO')]
